  ...
  <div class="row q-col-gutter-md">
            ...
            <div class="col">
              ...
              >
                ...
                <template v-slot:append>
                  <q-icon
                    v-if="productsTable.search !== ''"
                    name="close"
                    @click="productsTable.search = ''"
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td v-for="col in props.cols" :key="col.name" :props="props">
                  ...
                  <div v-else-if="col.field === 'file_size'">
      ...
      <q-card>
        <q-card-section>
          ...
          <p class="q-mt-sm">
            ...
            share the public link (and optionally the integrity code) with your
            buyer.
          </p>
          <!-- Commission info banner for sellers -->
          <q-banner
            v-if="adminSettings.commission_percent > 0"
          ...
          >
            ...
            /></template>
            A <strong>${ adminSettings.commission_percent }%</strong> platform
            ...
            <q-expansion-item group="extras" icon="info" label="How it works">
              <q-card>
                <q-card-section class="q-gutter-y-sm">
                  <p>
                    <strong>1. Create a product</strong> — upload your file and
                    set a price in sats.
                  </p>
                  <p>
                    <strong>2. Share</strong> the public link <em>and</em> the
                    SHA-256 hash privately with your buyer.
                  </p>
                  <p>
                    <strong>3. Buyer validates</strong> the hash, pays the
                    invoice, and downloads.
                  </p>
                  <p>
                    <strong>4. File stays protected</strong> — never accessible
    ...
    <q-dialog v-model="productDialog.show" position="top">
      <q-card
        ...
        class="q-pa-lg q-pt-md lnbits__dialog-card"
      ...
      >
        ...
        <q-checkbox
          v-model="productDialog.data.require_integrity"
          label="Require integrity code validation before payment"
          class="q-mb-md"
        >
        </q-checkbox>
      ...
      <q-card class="q-pa-lg" style="min-width: 500px; max-width: 90vw">
        <q-card-section>
          ...
          <div class="text-subtitle2 text-grey q-mb-md">
            ${ hashDialog.productName }
          </div>

          <q-banner class="bg-teal-1 text-teal-9 q-mb-md" rounded>
            <template v-slot:avatar
              ><q-icon name="security" color="teal"></q-icon
            ></template>
            Share both the <strong>SHA-256 hash</strong> and the
            <strong>public link</strong>
            privately with your buyer. The hash is used to unlock the payment.
          </q-banner>

          <div class="text-overline text-grey q-mb-xs">
            SHA-256 Integrity Hash
          </div>
          <div class="row items-center q-mb-md">
            <code
              class="col q-pa-sm bg-grey-2 rounded-borders"
            ...
            <q-btn
              ...
              class="q-ml-sm"
              @click="copyHash"
              :color="hashDialog.copied ? 'positive' : 'grey'"
            >
              <q-tooltip
                >${ hashDialog.copied ? 'Copied!' : 'Copy hash' }</q-tooltip
              >
            </q-btn>
          </div>
      ...
      >
        ...
        <q-card-section class="q-pa-none q-gutter-y-md">
          <q-input
            filled
            dense
            v-model.number="adminForm.commission_percent"
          ...
          >
            <template v-slot:append>
              <q-icon name="help_outline" class="cursor-pointer">
                <q-tooltip
                  >Set to 0 to disable. This percentage is automatically
                  deducted from each sale and sent to the commission
                  wallet.</q-tooltip
                >
              </q-icon>
            </template>
          ...
          <q-input
            filled
            dense
            v-model.number="adminForm.storage_quota_mb"
          ...
          >
            <template v-slot:append>
              <q-icon name="help_outline" class="cursor-pointer">
                <q-tooltip
                  >Maximum storage space allowed per user in megabytes. Default
                  is 1024 MB (1 GB).</q-tooltip
                >
              </q-icon>
            </template>
          </q-input>
          ...
          <div>
            <q-toggle
              v-model="adminForm.unlock_monthly"
              color="orange"
              :label="
                adminForm.unlock_monthly
                  ? 'Unlock: Monthly renewal'
                  : 'Unlock: One-time payment'
              "
            >
              <q-tooltip
                >OFF = sellers pay once forever. ON = sellers must renew every
                month.</q-tooltip
              >
            </q-toggle>
          </div>
    ...
    <q-dialog v-model="aboutDialog.show" position="top">
      <q-card
        class="q-pa-lg q-pt-md lnbits__dialog-card"
        style="min-width: 480px"
        v-if="aboutDialog.config"
      >
        <span class="text-h5 q-mb-md block"
          >${aboutDialog.config.name}
          <span class="text-caption text-grey"
            >v${aboutDialog.config.version}</span
          ></span
        >

        <q-card-section class="q-pa-none q-gutter-y-md">
          <p class="text-body2 text-grey-8">
            ${aboutDialog.config.short_description}
          </p>

          <div
            v-if="
              aboutDialog.config.images && aboutDialog.config.images.length > 0
            "
          >
            <div
              v-if="aboutDialog.config.images[0].uri"
              class="q-mb-md text-center"
            >
              <img
                :src="aboutDialog.config.images[0].uri"
                style="
                  max-width: 100%;
                  max-height: 200px;
                  object-fit: contain;
            ...
            <a
              v-if="aboutDialog.config.images[0].link"
              :href="aboutDialog.config.images[0].link"
              target="_blank"
          ...
          <div
            v-if="
              aboutDialog.config.contributors &&
              aboutDialog.config.contributors.length > 0
            "
          >
            <p class="text-subtitle2 q-mt-md q-mb-xs">Developed by:</p>
            <q-list dense>
              <q-item
                v-for="c in aboutDialog.config.contributors"
                :key="c.name"
                :href="c.uri"
                tag="a"
                target="_blank"
