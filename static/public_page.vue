  ...
  <div class="row q-col-gutter-md justify-center q-pt-lg">
    ...
    <div v-else-if="step === 'error'" class="col-12 col-sm-8 col-md-5">
      <q-card class="q-pa-lg text-center">
        <q-icon name="error_outline" size="4em" color="negative" class="q-mb-md"></q-icon>
      ...
      <div class="col-12 col-sm-10 col-md-5 col-lg-4 q-gutter-y-md">
        ...
        <q-card class="q-pa-md">
          <q-card-section class="q-pa-none q-mb-sm">
            ...
            <div class="text-caption text-grey">${ product.file_name } · ${ fileSizeFormatted } · ${ product.mime_type }</div>
          </q-card-section>
          ...
          <div class="row items-center">
            ...
            <span class="text-h5 text-bold q-ml-xs">${ product.price_sats.toLocaleString() } sats</span>
          </div>
        </q-card>
        ...
        <q-card v-if="step === 'integrity_required'" class="q-pa-md">
          ...
          <q-card-actions class="q-pa-none q-mt-md">
            <q-btn @click="submitIntegrity" unelevated color="teal"
              :loading="integrityLoading" class="full-width">
              Validate
            </q-btn>
          </q-card-actions>
        </q-card>
        ...
        <q-card v-else-if="step === 'integrity_ok'" class="q-pa-md">
          <q-card-section class="q-pa-none q-mb-md">
            <q-banner rounded class="bg-teal-1 text-teal-9">
              <template v-slot:avatar><q-icon name="check_circle" color="teal"></q-icon></template>
              Integrity code validated. You can now proceed to payment.
            </q-banner>
          </q-card-section>
          <q-btn @click="createInvoice" unelevated color="primary"
            icon="bolt" :loading="invoiceLoading" class="full-width">
        ...
        <q-card v-else-if="step === 'awaiting_payment'" class="q-pa-md text-center">
          <q-card-section class="q-pa-none q-mb-sm">
            <div class="text-subtitle2 q-mb-md">Scan or paste into your Lightning wallet</div>
            <lnbits-qrcode
            ...
            >
              <template v-slot:append>
                <q-btn flat dense icon="content_copy"
                  @click="() => { navigator.clipboard.writeText(paymentRequest); Quasar.Notify.create({type:'positive',message:'Copied!'}) }">
                </q-btn>
              </template>
            </q-input>
          </q-card-section>
          <div class="q-mt-md text-grey text-caption">
            <q-spinner-dots color="primary" size="1.2em" class="q-mr-xs"></q-spinner-dots>
        ...
        <q-card v-else-if="step === 'paid'" class="q-pa-md text-center">
          <q-icon name="check_circle" color="positive" size="4em" class="q-mb-sm"></q-icon>
          <div class="text-h6 q-mb-sm">Payment confirmed!</div>
          <div class="text-caption text-grey q-mb-lg">Your file is ready to download.</div>
          <q-btn @click="downloadFile" unelevated color="positive"
            icon="download" size="lg" class="full-width">
